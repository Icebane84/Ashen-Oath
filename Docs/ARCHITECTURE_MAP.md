...
- **Patch v158.19.0**: Master Cuts Granular Splitting & Martial AI StateTree Migration — `UAshenLiechtenauerMatrixDataAsset` data-driven counter matrix, `UAshenAIStateTreeEvaluator_Liechtenauer` $60\text{Hz}$ stance evaluator, and `UAshenAITask_ExecuteMasterCutCounter` StateTree combat execution.
- **Patch v158.18.0**: The Five Master Cuts & GAS Kinetic Combo Library — `UAshenGameplayAbility_CombatStrike` base class, the 5 Master Cuts (`UGA_AshenStrike_Zornhau`, `UGA_AshenStrike_Krumphau`, `UGA_AshenStrike_Zwerchhau`, `UGA_AshenStrike_Schielhau`, `UGA_AshenStrike_Scheitelhau`) + `UGA_AshenStrike_Mordhau`, 0-stamina cancel on Perfect Flow Glint, and non-modal guard branching.
- **Patch v158.17.0**: Campfire Reflection & Memory Integration Playloop — `UAshenCampfireReflectionSubsystem` sanctuary state machine crystallizing buffered imprints through the 4 Interpretive Lenses (Accountability, Grace/Compassion, Nihilism/Spite, Penance), zero-debt payoff ($Debt \to 0.0$), `UAshenOathbringerMorphComponent` live PBR metallurgical actuator, and `SAshenCampfireConstellationWidget`.
- **Patch v158.16.0**: CDTC-002 Canonical Source Manifest Integration ([`PRS-001-SRC-CDTC-002`](file:///c:/Users/Chris/Ashen%20Oath%20Unreal%20Engine/Docs/PRS-001-CDTC-001%20%28COMBAT%20DETERMINISM%20&%20TEMPORAL%20CONTRACT%29.md)) — Strict half-open Flow timing $[Apex, Apex + WindowDuration)$, Two-Phase relational commit aborting via `IAshenRelationalBridge`, `UNavigationSystemV1::TestPathSync` reachability, continuous mass formula ($45\text{kg}-120\text{kg}$), and 5-part boundary QA test suite.
